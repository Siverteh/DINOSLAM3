...
def repeatability_loss(h1: torch.Tensor, h2: torch.Tensor, xy1_f: torch.Tensor, xy2_f: torch.Tensor) -> torch.Tensor:
    """Encourage detector scores to be consistent at corresponding points."""
    s1 = torch.sigmoid(_grid_sample_map(h1, xy1_f))
    s2 = torch.sigmoid(_grid_sample_map(h2, xy2_f))
    return F.l1_loss(s1, s2)


def offset_regression_loss(offset: torch.Tensor, xy_f: torch.Tensor, xy_target_f: torch.Tensor) -> torch.Tensor:
    """offset predicts delta in feature coords (not pixels)."""
    pred = _grid_sample_desc(offset, xy_f)  # misuse function; offset is 2ch, normalized output
    # pred is normalized to unit length; not good. We'll do custom sampling:
    ...
    target_delta = (xy_target_f - xy_f).detach()
    return F.smooth_l1_loss(samp, target_delta)
...
def compute_losses(
    ...
    xi2 = x2.round().clamp(0, W-1).long()
    yi2 = y2.round().clamp(0, H-1).long()
    d2_obs = depth2[:,0, yi2, xi2]
    z2 = pts2[...,2]
    reproj_ok = (torch.abs(d2_obs - z2) < 0.05) | (d2_obs <= 0.0)  # tolerate missing depth
    mask = inb & reproj_ok
    ...
    # keep only valid
    # (we keep fixed N for simplicity, but mask out in losses)
    m = mask.float()
    ...
    rep = repeatability_loss(out1.heatmap, out2.heatmap, xy1_f, xy2_f)
    ...
    if out1.offset is not None:
        off_loss = offset_regression_loss(out1.offset, xy1_f, xy2_f)
