...
def contrastive_info_nce(
    ...
    # positives: dot product
    pos = (desc1 * desc2).sum(dim=-1) / temperature  # (B,N)

    # sample negative indices: (B,N,K)
    with torch.no_grad():
        neg_idx = torch.randint(0, N, (B, N, num_negatives), device=desc1.device)

    # gather negatives directly from desc2: use batch indexing
    # desc2: (B,N,D)
    b_idx = torch.arange(B, device=desc1.device).view(B, 1, 1).expand(B, N, num_negatives)  # (B,N,K)
...
def compute_losses(
    ...
    num_negatives = int(c_cfg["num_negatives"])

    # m: (B,N) float mask from geometry
    valid_flat = (m > 0.5)

    if valid_flat.sum() < 8:
        # not enough valid correspondences -> skip contrastive safely
        con = torch.tensor(0.0, device=rgb1.device)
    else:
        # flatten valid correspondences across batch
        d1v = d1[valid_flat]  # (Nv,D)
        d2v = d2[valid_flat]  # (Nv,D)

        # InfoNCE expects (B,N,D), so fake batch dim
        con = contrastive_info_nce(
            d1v.unsqueeze(0),
            d2v.unsqueeze(0),
            temperature,
            num_negatives,
