...
def load_dino_dpvo_config(path: str | Path) -> DinoDPVOConfig:
    path = Path(path).expanduser().resolve()
    data = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
    return DinoDPVOConfig(
        method_id=str(data.get("method_id", "dino_dpvo_v1")),
        feature_type=str(data.get("feature_type", "DINO_DPVO_V1")),
        raw=data,
    )
