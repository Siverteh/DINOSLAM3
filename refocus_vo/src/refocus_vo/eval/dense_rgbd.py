...
from refocus_vo.odometry import (
...
def evaluate_sequence(
    *,
    ...
    if dino_method_config is not None:
        if scorer is None:
            raise ValueError("dino_guided mode requires an initialized DinoStabilityScorer")
        vo = DinoGuidedVisualOdometryRgbdTensor(
            camera,
            scorer,
            keep_ratio=float(dino_method_config.keep_ratio),
            coarse_method=str(dino_method_config.coarse),
            refine_method=str(dino_method_config.refine),
            min_refine_pixels=int(dino_method_config.min_refine_pixels),
            mask_policy=str(dino_method_config.mask_policy),
            device=device,
        )
    else:
    ...
    try:
        for frame_idx, frame in enumerate(ds.frames):
            rgb = ds.read_rgb_np(frame["rgb"])
            depth = ds.read_depth_np(frame["depth"])
...
def main() -> None:
    ...
    ap.add_argument("--method", default="dino_guided")
    ap.add_argument("--method-manifest", default=str(DEFAULT_METHOD_MANIFEST))
    ...
    dino_method_config = None
    scorer = None
    if args.method not in {"hybrid", "point_to_plane"}:
        if args.method == "dino_guided":
            ...
            from refocus_vo.odometry.method_registry import DinoGuidedMethodConfig

            dino_method_config = DinoGuidedMethodConfig(
                method_id="dino_guided",
                feature_type="DINO_RGBD_GUIDED",
            ...
            dino_method_config = get_dino_method_config(args.method, args.method_manifest)
            layer_indices = tuple(int(v) for v in dino_method_config.layers)
        scorer = DinoStabilityScorer(
            ...
            keep_ratio=float(dino_method_config.keep_ratio),
            min_keep_pixels=int(dino_method_config.min_refine_pixels),
            weights=dict(dino_method_config.weights),
        )

    feature_label = {
        "hybrid": "RGBD_TENSOR_HYBRID",
        "point_to_plane": "RGBD_TENSOR_POINT_TO_PLANE",
    }.get(args.method, dino_method_config.feature_type if dino_method_config is not None else str(args.method).upper())
    ...
    for seq in sequences:
        status, metrics = evaluate_sequence(
            ...
            scorer=scorer,
            dino_method_config=dino_method_config,
