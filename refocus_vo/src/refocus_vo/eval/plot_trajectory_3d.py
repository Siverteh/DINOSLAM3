...
def load_plot_xyz(groundtruth: Path, trajectory: Path, max_dt: float, *, correct_scale: bool = False):
    ...
    if traj_est_assoc.num_poses >= 2:
        rot, trans, scale = traj_est_assoc.align(traj_ref_assoc, correct_scale=bool(correct_scale))
...
def main():
    ...
    ap.add_argument("--correct-scale", action="store_true")
    args = ap.parse_args()

    gt_path = Path(args.groundtruth)
    traj_path = Path(args.trajectory)
    try:
        gt_xyz, est_xyz = load_plot_xyz(
            gt_path,
            traj_path,
            args.max_dt,
            correct_scale=bool(args.correct_scale),
        )
    except Exception:
