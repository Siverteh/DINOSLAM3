...
def load_tum_camera(sequence: str, settings_dir: str | Path | None = None) -> CameraShim:
    settings_root = (
        ...
        else Path(__file__).resolve().parents[4] / "pyslam" / "settings"
    )
...
class DinoGuidedVisualOdometryRgbdTensor(VisualOdometryRgbdTensor):
    ...
    def __init__(
        ...
        *,
        ...
        self.mask_policy = str(mask_policy).strip().lower()
    ...
    def process_frame(self, frame_id) -> None:
        ...
        prev_pixel_score = prev_map.pixel_score
        current_pixel_score = current_map.pixel_score
        prev_mask = prev_map.pixel_mask
        current_mask = current_map.pixel_mask

        if self.mask_policy == "symmetric_consensus":
            _, prev_pixel_score, prev_mask = self.scorer.build_consensus_mask(
                self.prev_observation,
                prev_map,
                current_map,
                coarse_transform,
                self._intrinsics_np,
                keep_ratio=self.keep_ratio,
                min_keep_pixels=self.min_refine_pixels,
                fallback_mask=prev_map.pixel_mask,
            )
            _, current_pixel_score, current_mask = self.scorer.build_consensus_mask(
                current_observation,
                current_map,
                prev_map,
                np.linalg.inv(coarse_transform),
                ...
                keep_ratio=self.keep_ratio,
                min_keep_pixels=self.min_refine_pixels,
                fallback_mask=current_map.pixel_mask,
            )

        prev_kept = int(prev_mask.sum().item())
        current_kept = int(current_mask.sum().item())
        ...
        self.last_stability_stats = {
            ...
            "prev_keep_ratio": float(prev_kept) / float(prev_mask.numel()),
            "current_keep_ratio": float(current_kept) / float(current_mask.numel()),
            "refine_ok": 1.0 if refine_ok else 0.0,
            "prev_mean_score": float(prev_pixel_score.mean().item()),
            "current_mean_score": float(current_pixel_score.mean().item()),
        }
