...
MANIFEST_PATH="${MANIFEST_PATH:-$SUBTREE_ROOT/configs/dino_dpvo_focus071_lr_only_sweep_v1.yaml}"
BASE_OUTPUT_DIR="${BASE_OUTPUT_DIR:-$SUBTREE_ROOT/runs/sweeps/dino_dpvo_focus071_lr_only_sweep_v1}"
...
if [[ "$DETACH" -eq 1 && "${RUN_DETACHED_CHILD:-0}" != "1" ]]; then
  ...
  echo "Detached focus071 LR-only sweep started."
