...
MANIFEST_PATH="${MANIFEST_PATH:-$SUBTREE_ROOT/configs/dino_dpvo_freiburg1_paper_runtime_sweep_v1.yaml}"
BASE_OUTPUT_DIR="${BASE_OUTPUT_DIR:-$SUBTREE_ROOT/runs/sweeps/dino_dpvo_freiburg1_paper_runtime_sweep_v1}"
...
if [[ "$DETACH" -eq 1 && "${RUN_DETACHED_CHILD:-0}" != "1" ]]; then
  if [[ -f "$PID_PATH" ]]; then
    ...
    if [[ -n "$EXISTING_PID" ]] && kill -0 "$EXISTING_PID" 2>/dev/null; then
      echo "Runtime sweep runner already active: PID $EXISTING_PID"
      echo "Log: $LOG_PATH"
      echo "Leaderboard: $BASE_OUTPUT_DIR/leaderboard_pack.csv"
      exit 0
    fi
  fi
  (
  ...
  echo "Detached Freiburg1 runtime sweep v1 started."
  echo "Sweep root: $BASE_OUTPUT_DIR"
  echo "Runner PID: $NEW_PID"
  echo "Watch log:"
  echo "tail -f $LOG_PATH"
  echo "Watch leaderboard:"
  echo "tail -f $BASE_OUTPUT_DIR/leaderboard_pack.csv"
  exit 0
fi

cd "$REPO_ROOT"
export PYTHONPATH="$SUBTREE_ROOT/src${PYTHONPATH:+:$PYTHONPATH}"
"$PYTHON_BIN" -m refocus_vo.sweeps.run_runtime_pack_sweep \
