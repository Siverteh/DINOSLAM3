...
def main():
    ...
    model = LocalFeatureNet(
        ...
        head_variant=str(cfg["model"]["heads"].get("variant", "v1")),
        head_channels=cfg["model"]["heads"].get("head_channels", None),
        head_tower_depth=int(cfg["model"]["heads"].get("tower_depth", 2)),
        head_norm=str(cfg["model"]["heads"].get("norm", "group")),
        head_act=str(cfg["model"]["heads"].get("act", "silu")),
        detector_logit_scale=float(cfg["model"]["heads"].get("detector_logit_scale", 1.0)),
        reliability_logit_scale=float(cfg["model"]["heads"].get("reliability_logit_scale", 1.0)),
        fusion_variant=str(cfg["model"].get("fusion", {}).get("variant", "v1")),
        fusion_channels=int(cfg["model"].get("fusion", {}).get("channels", 256)),
    ).to(device).eval()
