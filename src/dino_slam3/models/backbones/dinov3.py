...
class DinoTokens:
    ...
    hidden_states: Dict[int, torch.Tensor] | None = None  # encoder block idx -> (B, C, H/ps, W/ps)
...
class DinoV3Backbone(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        patch_tokens = self._extract_patch_tokens(tokens)  # (B, N, C)
        ...
        if return_hidden_states:
            ...
            if raw_hidden_states is None:
                raise RuntimeError(
                    ...
                    "The dense RGB-D stability path requires output_hidden_states support."
                )
            requested = tuple(int(i) for i in (hidden_state_indices or ()))
            hidden_states_out = {}
            for block_idx in requested:
                tuple_idx = int(block_idx) + 1  # hidden_states[0] is the patch embedding output.
        ...
        return DinoTokens(tokens=patch_tokens, hidden_states=hidden_states_out)
