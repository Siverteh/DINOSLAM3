...
DinoSlam3Feature2D = import_from("pyslam.local_features.feature_dinoslam3", "DinoSlam3Feature2D")
    ...
    def __init__(
        ...
        **kwargs,  # Collect all remaining keyword arguments
        ...
        if (
            ...
            #
            #
        elif self.detector_type == FeatureDetectorTypes.DINOSLAM3:
            self.num_levels = 1
            self.need_color_image = True
            self.oriented_features = False
            ckpt = kwargs.get('dinoslam3_checkpoint')
            if ckpt is None:
                ckpt = os.environ.get("DINOSLAM3_CKPT") or os.environ.get("CKPT")
            dev = kwargs.get('dinoslam3_device', 'cuda')
            nf  = kwargs.get('dinoslam3_num_features', self.num_features)
            norm = kwargs.get('dinoslam3_normalize_descriptors', True)
            if ckpt is None:
                raise ValueError(
                    "DINOSLAM3 detector requires 'dinoslam3_checkpoint' in config "
                    "or CKPT/DINOSLAM3_CKPT environment variable"
                )
            self._feature_detector = DinoSlam3Feature2D(
                checkpoint_path=ckpt,
                device=dev,
                num_features=nf,
                normalize_descriptors=norm,
            )
        ...
        if self.is_detector_equal_to_descriptor:
            ...
            if (
                ...
                #
                #
            elif self.descriptor_type == FeatureDescriptorTypes.DINOSLAM3:
                self.oriented_features = False
                self.need_color_image = True
                if self.detector_type != FeatureDetectorTypes.DINOSLAM3:
                    raise ValueError(
                        "You cannot use DINOSLAM3 descriptor without DINOSLAM3 detector!\nPlease, select DINOSLAM3 as both descriptor and detector!"
                    )
                self._feature_descriptor = self._feature_detector
                #
                #
