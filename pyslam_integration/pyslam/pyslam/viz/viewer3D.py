...
if _HEADLESS:
    class Viewer3D:
        """
        Headless stub: keeps the rest of the pipeline alive without Pangolin/X11.
        Methods exist so callers don't need special-casing everywhere.
        """
        def __init__(self, *args, **kwargs):
            print("[Viewer3D] Headless mode: Viewer3D disabled (no DISPLAY).")

        def start(self): pass
        def stop(self): pass
        def join(self): pass
        def is_ready(self): return True
        def set_gt_trajectory(self, *args, **kwargs): pass
        def push_frame(self, *args, **kwargs): pass
        def push_keyframe(self, *args, **kwargs): pass
        def push_map(self, *args, **kwargs): pass
        def push_trajectory(self, *args, **kwargs): pass
        def update(self, *args, **kwargs): pass

else:
    # --- keep the original Pangolin-based implementation below this line ---
