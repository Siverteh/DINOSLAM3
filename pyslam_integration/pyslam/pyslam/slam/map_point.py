...
class MapPointBase(object):
    ...
    def remove_frame_view(self, frame: "Frame", idx=None):
        ...
        with self._lock_features:
            ...
            if idx is not None:
                # Python-core cleanup may clear frame.points[idx] before calling here.
                # Keep this robust by validating only when a non-null mismatch exists.
                if __debug__:
                    cur_match = frame.get_point_match(idx)
                    if cur_match is not None and cur_match is not self:
                        Printer.orange(
                            f"MapPoint.remove_frame_view: inconsistent match at frame {frame.id}, idx {idx}"
                        )
