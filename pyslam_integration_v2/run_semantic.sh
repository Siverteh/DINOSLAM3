...
for sequence in "${ACTIVE_SEQUENCES[@]}"; do
  ...
  seq_folder="$DATA_PATH/rgbd_dataset_${sequence}"
  ...
  rm -f "$run_output"/trajectory_*.txt
  ...
  "$PYTHON_BIN" - "$setting_template" "$run_settings" "$NFEATURES" "$CKPT" <<'PY' >> "$log_file" 2>&1
import sys
import yaml

base_settings, out_settings, nfeatures, ckpt = sys.argv[1], sys.argv[2], int(sys.argv[3]), sys.argv[4]
...
PY
  ...
  "$PYTHON_BIN" - "$run_cfg" "$DATA_PATH" "$sequence" "$run_settings" "$run_output" <<'PY' >> "$log_file" 2>&1
import sys
import yaml

cfg_path, data_path, sequence, settings_path, output_path = sys.argv[1:6]
cfg = {
    "DATASET": {"type": "TUM_DATASET"},
    "TUM_DATASET": {
        "type": "tum",
        "sensor_type": "rgbd",
        "base_path": data_path,
    ...
    "GLOBAL_PARAMETERS": {
        ...
        "kUseDepthEstimatorInFrontEnd": False,
    },
}
...
PY
  ...
  raw_traj="$run_output/trajectory_final.txt"
  if [[ -s "$raw_traj" ]]; then
    cp "$raw_traj" "$out_traj"
...
PY
  ...
  append_row "$sequence" "DINOSLAM3" "ok" "$ate_rmse" "$ate_mean" "$ate_median" "$rpe_trans_rmse" "$rpe_rot_rmse"
