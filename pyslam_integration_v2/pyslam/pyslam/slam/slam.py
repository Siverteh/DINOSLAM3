...
try:
    ...
    semantic_mapping_factory = None
    SemanticMappingShared = None
...
try:
    ...
    VolumetricIntegrationOutput = None
    VolumetricIntegratorType = None
    volumetric_integrator_factory = None
...
class Slam(object):
    ...
    def init_semantic_mapping(self, semantic_mapping_config, headless=False):
        if (
            ...
            and semantic_mapping_factory is not None
        ):
            if self.semantic_mapping is not None:
                self.semantic_mapping.quit()
        ...
        elif Parameters.kDoSparseSemanticMappingAndSegmentation and semantic_mapping_config is not None:
            Printer.orange("SLAM: semantic mapping requested but semantics modules are unavailable.")
    ...
    def init_volumetric_integrator(self):
        if Parameters.kDoVolumetricIntegration and volumetric_integrator_factory is not None:
        ...
        elif Parameters.kDoVolumetricIntegration:
            Printer.orange("SLAM: volumetric integration requested but dense modules are unavailable.")
    ...
    def load_system_state(self, path):
        ...
        with open(map_file_path, "rb") as f:
            ...
            if self.slam_mode == SlamMode.SLAM:
                ...
                if semantic_mapping_config is not None and SemanticMappingShared is not None:
