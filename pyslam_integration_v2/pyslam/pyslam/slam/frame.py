...
class Frame(FrameBase):
    ...
    def __init__(self, *args, **kwargs):
        # Support both constructor signatures seen across fork variants:
        # 1) Frame(camera, img, img_right=..., depth=..., ...)
        # 2) Frame(img, img_right, depth, camera, ...)
        pose = kwargs.pop("pose", None)
        id = kwargs.pop("id", None)
        timestamp = kwargs.pop("timestamp", None)
        img_id = kwargs.pop("img_id", None)
        is_keyframe = kwargs.pop("is_keyframe", False)
        use_threading = kwargs.pop("use_threading", True)
        img = kwargs.pop("img", None)
        img_right = kwargs.pop("img_right", None)
        depth = kwargs.pop("depth", None)
        camera = kwargs.pop("camera", None)
        semantic_img = kwargs.pop("semantic_img", None)
        semantic_instances_img = kwargs.pop("semantic_instances_img", None)
        frame_data_dict = kwargs.pop("frame_data_dict", None)
        if kwargs:
            raise TypeError(f"Frame.__init__ got unexpected keyword arguments: {list(kwargs.keys())}")

        if len(args) >= 1 and isinstance(args[0], Camera):
            camera = args[0]
            if len(args) >= 2:
                img = args[1]
            if len(args) >= 3:
        ...
        if img is not None and img.size == 0:
            raise ValueError("Frame: empty image")
        if camera is None:
            raise ValueError("Frame: camera is None")

        FrameBase.__init__(self, camera=camera, pose=pose, id=id, timestamp=timestamp, img_id=img_id)
        ...
        self.depth_factor = camera.depth_factor if camera is not None else None
        ...
        self.u_right = None  # legacy alias
        self.kps_ur = None
        self.depths = None
        self.depths_r = None
        ...
        self.outliers_kps = None
        self.outliers_pts = None
        self.outliers = None

        self.points = None
        self.points_matched = None
        self.points_fov = None
        self.kf_ref = None

        self.keypoint_filter_name = ""

        self._kd = None

        self.is_ok = True
        self.desc_is_ok = False

        self.is_compute_des = True
        ...
        # Loading from serialized data bypasses feature extraction.
        if frame_data_dict is not None:
            for key, value in frame_data_dict.items():
                setattr(self, key, value)
            if self.points is None:
                self.points = []
            return

        # KeyFrame constructor passes img=None to avoid recomputing features.
        if img is None:
            if self.points is None:
                self.points = []
            return
        ...
        # -------------------------
        # IMPORTANT: build a depth-valid mask for DINOSLAM3 extraction
        # -------------------------
        mask_for_features = None
        try:
            if (
                depth is not None
                and FeatureTrackerShared.feature_manager is not None
                and FeatureTrackerShared.feature_manager.detector_type == FeatureDetectorTypes.DINOSLAM3
            ):
                # valid depth pixels only
                mask_for_features = (depth > 0).astype(np.uint8) * 255
                mask_for_features = np.ascontiguousarray(mask_for_features)
        except Exception:
            mask_for_features = None

        def create_img_pyramid(img):
            if FeatureTrackerShared.feature_manager.is_detector_equal_to_descriptor:
                assert FeatureTrackerShared.feature_manager.detector is not None
                FeatureTrackerShared.feature_manager.detector.detect(img)
            else:
                assert FeatureTrackerShared.feature_manager.detector is not None
                FeatureTrackerShared.feature_manager.detector.detect(img)
        ...
        def detect_and_compute(img, left=True, mask=None):
            if left:
                if self.is_compute_des:
                    kps, des = FeatureTrackerShared.feature_tracker.detectAndCompute(img, mask=mask)
                else:
                    kps = FeatureTrackerShared.feature_tracker.detect(img, mask=mask)
                    des = None
            else:
                if self.is_compute_des:
                    kps, des = FeatureTrackerShared.feature_tracker.detectAndCompute_right(img, mask=mask)
                else:
                    kps = FeatureTrackerShared.feature_tracker.detect_right(img, mask=mask)
                    des = None
            return kps, des

        if img_right is None:
            # monocular or RGBD
            self.kps, self.des = FeatureTrackerShared.feature_tracker.detectAndCompute(img, mask=mask_for_features)
        else:
            if use_threading:
                from multiprocessing.pool import ThreadPool

                pool = ThreadPool(processes=2)
                async_result1 = pool.apply_async(detect_and_compute, (img, True, mask_for_features))
                async_result2 = pool.apply_async(detect_and_compute, (img_right, False, None))
                self.kps, self.des = async_result1.get()
                self.kps_r, self.des_r = async_result2.get()
                pool.close()
                pool.join()
            else:
                self.kps, self.des = FeatureTrackerShared.feature_tracker.detectAndCompute(img, mask=mask_for_features)
                self.kps_r, self.des_r = FeatureTrackerShared.feature_tracker.detectAndCompute_right(img_right, mask=None)

        self.keypoint_filter_name = FeatureTrackerShared.feature_manager.keypoint_filter_type.name
        ...
        if self.kps_r is not None:
            self.kps_r = np.array(self.kps_r)
        if self.des_r is not None:
            self.des_r = np.array(self.des_r)

        if self.kps is not None and self.des is not None:
            self.desc_is_ok = True
            self.compute_kpsn_kpsu()
            self.compute_stereo(depth)
        else:
            self.desc_is_ok = False

        if self.kps is not None:
            if self.outliers_kps is None:
                self.outliers_kps = np.zeros(len(self.kps), dtype=bool)
            if self.points_matched is None:
                self.points_matched = np.full(len(self.kps), None)
            if self.points is None or len(self.points) != len(self.kps):
                self.points = np.full(len(self.kps), None, dtype=object)
            if self.outliers is None or len(self.outliers) != len(self.kps):
                self.outliers = np.full(len(self.kps), False, dtype=bool)
    ...
    @staticmethod
    def _extract_keypoint_arrays(kps):
        if kps is None:
            return None, None, None, None
        if len(kps) == 0:
            empty_f = np.zeros((0,), dtype=np.float32)
            empty_i = np.zeros((0,), dtype=np.int32)
            return np.zeros((0, 2), dtype=np.float32), empty_f, empty_f, empty_i

        kps_list = list(kps)
        first = kps_list[0]

        if hasattr(first, "pt"):
            pts = np.array([kp.pt for kp in kps_list], dtype=np.float32)
            sizes = np.array([kp.size for kp in kps_list], dtype=np.float32)
            angles = np.array([kp.angle for kp in kps_list], dtype=np.float32)
            octaves = np.array([kp.octave for kp in kps_list], dtype=np.int32)
            return pts, sizes, angles, octaves

        arr = np.asarray(kps_list)
        if arr.ndim == 1:
            arr = arr.reshape(-1, 1)
        if arr.shape[1] < 2:
            raise ValueError("Frame: invalid keypoint array format")

        pts = np.ascontiguousarray(arr[:, :2], dtype=np.float32)
        sizes = (
            np.ascontiguousarray(arr[:, 2], dtype=np.float32)
            if arr.shape[1] > 2
            else np.ones((arr.shape[0],), dtype=np.float32)
        )
        angles = (
            np.ascontiguousarray(arr[:, 3], dtype=np.float32)
            if arr.shape[1] > 3
            else np.zeros((arr.shape[0],), dtype=np.float32)
        )
        octaves = (
            np.ascontiguousarray(arr[:, 5], dtype=np.int32)
            if arr.shape[1] > 5
            else np.zeros((arr.shape[0],), dtype=np.int32)
        )
        return pts, sizes, angles, octaves

    def compute_kpsn_kpsu(self):
        # Convert keypoints into numeric arrays expected by the rest of the SLAM pipeline.
        kps, sizes, angles, octaves = self._extract_keypoint_arrays(self.kps)
        self.kps = np.ascontiguousarray(kps) if kps is not None else None
        self.sizes = sizes
        self.angles = angles
        self.octaves = octaves
        if self.kps is not None:
            self.kpsu = np.ascontiguousarray(self.camera.undistort_points(self.kps))
            self.kpsn = np.ascontiguousarray(self.camera.unproject_points(self.kpsu))
        else:
            self.kpsu = None
            self.kpsn = None

        if self.kps_r is not None:
            kps_r, sizes_r, angles_r, octaves_r = self._extract_keypoint_arrays(self.kps_r)
            self.kps_r = np.ascontiguousarray(kps_r)
            self.sizes_r = sizes_r
            self.angles_r = angles_r
            self.octaves_r = octaves_r
            # Right image is rectified in stereo pipelines.
            self.kpsu_r = np.ascontiguousarray(self.kps_r)
            self.kpsn_r = np.ascontiguousarray(self.camera.unproject_points(self.kpsu_r))
    ...
    def compute_stereo(self, depth):
        if self.kpsu is None:
            self.kps_ur = None
            self.depths = None
            return

        num_kps = len(self.kpsu)
        self.kps_ur = np.full((num_kps,), -1.0, dtype=np.float32)
        self.depths = np.full((num_kps,), -1.0, dtype=np.float32)
        self.u_right = self.kps_ur

        if depth is not None:
            depth_f = depth.astype(np.float32)
            if self.depth_factor is not None and self.depth_factor != 1.0:
                depth_f = depth_f * float(self.depth_factor)
            self.compute_stereo_from_rgbd(self.kps, depth_f)
            self.u_right = self.kps_ur
        elif self.kps_r is not None and self.img is not None and self.img_right is not None:
            self.compute_stereo_matches(self.img, self.img_right)
            self.u_right = self.kps_ur
